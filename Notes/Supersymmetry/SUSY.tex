...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{SUSY in \secmath{d=0} dimensions}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\eq{
	\exp \left[ -\sum_{i=0}^N \frac{\Delta \tau}{2} \left( \frac{x_{i+1} - x_i}{\Delta \tau}\right)^2 \right] \to \exp \left(  -\int_0^\beta \frac{1}{2} \dot{x}^2 d\tau \right)
}
Then we heuristically obtain the \emph{path integral representation}
\eq{
	\braket{y_1 | e^{-\beta H} | y_0 } = \int_{\mc{C}_\beta[y_1,y_0]} e^{-\int_0^\beta \frac{1}{2} \dot{x}^2 d\tau} \, \mc{D}x
}
...
\eq{
	...
	&= \int \left[ \int_{\mc{C}_\beta[y,y]} e^{-S[x]} \, \mc{D}x \right] dy \\
...
\eq{
	...
	\bra{\bar{\eta}} &= \bra{0} e^{\hat{\psi}\bar{\eta}} 
} 
\begin{prop}
...
and 
\eq{
	S\tr  ( e^{-\beta H} ) = \tr_\mc{H} ( (-1)^F e^{-\beta H} ) &= \int \int \braket{\bar{\chi}^\prime | e^{-\beta H} | \chi } e^{-\bar{\chi}\chi} d^2\chi \\
	&= \underbrace{\int \exp\left( - S[\bar{\psi},\psi] \right) \, \mc{D} \psi \, \mc{D} \bar{\psi}}_{\text{periodic boundary conditions}}
}
...
\begin{definition}
	The \bam{Fermionic number operator} is $\hat{F} = \hat{\bar{\psi}}\hat{\psi}$
\end{definition}
\begin{prop}
...
\eq{
Q : \mc{H}_{F,n} \to \mc{H}_{B,n}, \quad Q(\mc{H}_{B,n}) = 0 \\
\bar{Q} : \mc{H}_{B,n} \to \mc{H}_{F,n}, \quad \bar{Q}(\mc{H}_{F,n}) = 0
}
...
\eq{
\int \exp(-S_E[\bar{\psi},\psi]) \mc{D}\psi \, \mc{D}\bar{\psi}
}
where 
\eq{
...
\eq{
\frac{d}{d\lambda} I_w ( \lambda) = - \int_P \left[ \oint_{S^1} \lambda (\del h)^2 + \del^2 h \bar{\psi} \psi \, d\tau \right] e^{-S_E[x,\psi,\bar{\psi}]} \, \mc{D}x \, \mc{D} \psi \, \mc{D} \bar{\psi}
}
However 
\eq{
...
\eq{
S_E^{(2)} = \oint \psquare{\frac{1}{2}\delta x \left( -\frac{d^2}{d\tau^2} + h^{\prime\prime}(x_\ast)^2 \right) \delta x + \bar{\psi}\left( \frac{d}{d\tau} + h^{\prime\prime}(x_\ast) \right)\psi} d\tau 
}
...
&= \frac{\prod_{n\in\mbb{Z}}(\frac{2\pi i n }{\beta}+h^{\prime\prime}(x_\ast))}{\sqrt{\prod_{n\in\mbb{Z}}((\frac{2\pi n}{\beta})^2+h^{\prime\prime}(x_\ast)^2)}} \\
&= \frac{h^{\prime\prime}(x_\ast)}{|h^{\prime\prime}(x_\ast)|} \; \text{(only $n=0$ terms don't cancel)}
}
Hence 
\eq{
I_w = \sum_{x_\ast : h^\prime(x_\ast)=0}  \frac{h^{\prime\prime}(x_\ast)}{|h^{\prime\prime}(x_\ast)|}
}
...
We now let our bosonic field $x$ define a map $x:M\to N$ from our worldline $M$ to a compact $n$-dimensional Riemannian manifold $(N,g)$. We call $N$ the \bam{target space}. We often let $x^a$ be coordinates on $U\subset N$ open and $x^a(\tau)$ be the corresponding field. Choose the following action:
\begin{definition}
	The \bam{Non-Linear Sigma Model (NLSM) action} is 
	\eq{
...
where 
\eq{
\Gamma^a_{bc} = \frac{1}{2} g^{ad}\psquare{\del_b g_{cd} + \del_c g_{bd} - \del_d g_{bc}}
}
\end{prop}
\begin{proof}
\eq{
\delta S &= \int_M [ g_{ab} \dot{x}^a \frac{d(\delta x^b)}{d\tau} + \frac{1}{2} \del_c g_{ab} \dot{x}^a \dot{x}^b \delta x^c] d\tau \\
&= \int_M \left[-\frac{d}{d\tau} \left( g_{ab} \dot{x}^a \right) + \frac{1}{2} \del_c g_{ab} \dot{x}^b \dot{x}^c   \right] \delta x^c d\tau + g_{ab}(x) \dot{x}^a \delta x^b |_{\delta M}
}
...
\eq{
\hat{H} &= -\frac{1}{2}  \underbrace{\frac{1}{\sqrt{g}} \pd{x^a}\left( g^{ab}\sqrt{g} \pd{x^b} \right)}_{\text{usual Laplacian in curved space}} + \alpha R[g] \quad \text{for $\alpha \in \mbb{R}$} \\
&= -\frac{1}{2} \nabla^a \nabla_a + \alpha R
}
...
\eq{
g^{ef} \pd{x^f} \left( dx^a \wedge dx^b \wedge \dots \wedge dx^c \right) 
}
\end{remark}

\begin{definition}
Define the \bam{Hodge star} operator $\star : \Omega^p(N) \to \Omega^{n-p}(N)$ by acting on 
\eq{
\omega = \frac{1}{p!} \omega_{a_1 \dots a_p} dx^{a_1} \wedge \dots \wedge dx^{a_p} 
}
\eq{
	\star \omega = \frac{1}{p! (n-p)!}\sqrt{g} \eps\indices{^{a_1}^\dots^{a_p}_{b_{p+1}}_\dots_{b_n}} \omega_{a_1 \dots a_p} dx^{b_{p+1}} \wedge \dots \wedge dx^{b_n}
}
...
\eq{
Q = i\bar{\psi}^a p_a &\to d \\
\bar{Q} = -i\psi^a p_a &\to d^\dagger = (-1)^{n(p+1)+1} \star d \star
}
...
\eq{
H = \frac{1}{2} \left( d^\dagger d + d d^\dagger \right) 
}
\begin{definition}
	Define the \bam{Laplacian} $\Delta : \Omega^p(N) \to \Omega^p(N)$ as
	\eq{
...
\begin{prop}
	For $f \in \Omega^0(N)$, 
\eq{
\Delta f = \frac{1}{\sqrt{g}} \del_a \pround{\sqrt{g} g^{ab} \del_b f}
}
\end{prop}
\begin{proof}
	\begin{comment}
...
&= \star d(\star df) \\
&= \star d \left( \frac{\sqrt{g}}{(n-1)!} g^{ab} \del_a f \eps_{bc\dots d} \underbrace{dx^c \wedge \dots \wedge dx^d}_{n-1} \right) \\
&= \star \left[ \frac{1}{(n-1)!} \del_m \left( \sqrt{g} g^{ab} \del_a f \right) \eps_{bc\dots d} \underbrace{dx^m \wedge dx^c \wedge \dots \wedge dx^d}_{n} \right] \\
...
\end{proof}
\begin{definition}
	A form $\omega$ is called \bam{harmonic} if $\Delta \omega = 0$. 
\end{definition}
...
\begin{definition}
	The \bam{p\textsuperscript{th} homology group} is
	\eq{
...
It is also known 
\eq{
\chi(S^n) &=1+(-1)^n =  \left\{ \begin{array}{cc} 2 & \text{n even} \\ 0 & \text{n odd} \end{array} \right.  \\
\chi(\Sigma_g) &= 2-2g
}
\end{example}
...
\eq{
S &= \oint \left( \frac{1}{2} g_{ab} \dot{x}^a \dot{x}^b + g_{ab} \bar{\psi}^a \nabla_\tau \psi^b - \frac{1}{2} R_{abcd} \bar{\psi}^a \psi^b \bar{\psi}^c \psi^d \right) \, d\tau \\
...
\eq{
\delta x^a(\tau) = \sum_{k \neq 0} \delta x_k^a(\tau) e^{\frac{2\pi i k \tau}{\beta}} 
}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Spinors in \secmath{n=2m} Dimensions}
...
\eq{
\slashed{\del} = \begin{pmatrix} 0 & \del^+ \\ \del^- & 0 \end{pmatrix}
}
where $\del^\pm : S^\pm \to S^\mp$. Note that $\del^\pm$ annihilates $S^\mp$, so $(\del^\pm)^2 = 0$.
\begin{definition}
	The \bam{index} of the chiral Dirac operator $\del^+$ is  
\eq{
...
\eq{
g_{ab}(x) &= \delta_{ab} - \frac{1}{3} R_{abcd}(x_0)\delta x^c \delta x^d + \mc{O}(\delta x^3) \\
\Gamma^a_{bc} (x) &= \del_d \Gamma^a_{bc}(x_0) \delta x^d + \mc{O}(\delta x^2) = - \frac{1}{3} \left[ R\indices{^a_b_c_d} (x_0) + R\indices{^a_c_b_d}(x_0) \right] \delta x^d + \mc{O}(\delta x^2)
}
To second order in the fluctuations, the actions becomes 
\eq{
S^{(2)}[x_0, \psi_0, \delta x, \delta \psi] = \oint - \frac{1}{2} \delta x_a \frac{d^2}{d\tau^2} \delta x^a + \frac{1}{2} \delta \psi_a \frac{d}{d\tau} \delta \psi^a - \frac{1}{4} R_{abcd} \psi_0^a \psi_0^b \delta x^c \delta \dot{x}^d \, d\tau
}
...
\eq{
\int e^{-S[x_0,\psi_0,\delta x, \delta \psi]} \, \mc{D}(\delta x) \, \mc{D}(\delta \psi) = \frac{\sqrt{\det^\prime ( \del_\tau \delta_b^a)}}{\sqrt{\det^\prime\pround{-\del_\tau^2 + \mc{R}\indices{^a_b}(x_0,\psi_0) \del_\tau}}} = \frac{1}{\sqrt{\det^\prime \pround{-\del_\tau \delta^a_b + \mc{R}\indices{^a_b}}}}
}
...
\eq{
\prod_{k=1}^\infty (2\pi k)^4 = (4\pi^2)^{\zeta(0)} e^{-2 \zeta^\prime(0)} = 1 
}
...
we recall 
\eq{
\sinh(z) = z\prod_{k = 1}^\infty \psquare{1 + \frac{z^2}{\pi^2 k^2}}
}
so we have 
\eq{
\sqrt{\det^\prime ( -D_i)} = \frac{\sinh\pround{\sfrac{\omega_i}{2}}}{\pround{\sfrac{\omega_i}{2}}}
}
\begin{theorem}
	The \bam{Atiyah-Singer Index Theorem} says that 
	\eq{
\ind(D^+) = \int_N \det^\frac{1}{2} \pround{\frac{\sfrac{\mc{R}}{4\pi}}{\sinh \sfrac{\mc{R}}{4\pi}}}	
}
...
\eq{
I_w = \ind(D^+) &= \frac{1}{(2\pi)^m}\int \prod_{i=1}^m \frac{\pround{\sfrac{\omega_i}{2}}}{\sinh\pround{\sfrac{\omega_i}{2}}}  \, d^nx_0 \, d^n\psi_0 \\ 
&=\int \prod_{i=1}^m \frac{\pround{\sfrac{\omega_i}{4\pi}}}{\sinh\pround{\sfrac{\omega_i}{4\pi}}}  \, d^nx_0 \, d^n\psi_0 \\
&= \int \det \pround{\frac{\sfrac{\mc{R}\indices{^a_b}(x_0,\psi_0)}{4\pi}}{\sinh\pround{\sfrac{\mc{R}\indices{^a_b}(x_0,\psi_0)}{4\pi}}}} \, d^nx_0 \, d^n\psi_0 \\
&= \int_N \det \pround{\frac{\sfrac{\mc{R}}{4\pi}}{\sinh \sfrac{\mc{R}}{4\pi}}}
}
\end{proof}  
...
\eq{
S[\psi] = \frac{1}{2\pi} \int_{\mbb{R}^2} i  \bar{\psi} \slashed{\del} \psi \, dt \, ds 
}
where $\slashed{\del} = \gamma^\mu \del_\mu$ and $\bar{\psi} = \psi^\dagger \gamma^t$. Hence 
\eq{
S[\psi] = \frac{1}{2\pi} \int_{\mbb{R}^2} \psquare{i \bar{\psi}_- \pround{\del_t + \del_s} \psi_- +  i \bar{\psi}_+ \pround{\del_t - \del_s} \psi_+ } \, dt\, ds
}
Classically, 
\eq{
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Superspace in \secmath{d=2}}
...
\eq{
\delta = \eps_+ Q_- - \eps_- Q_+ - \bar{\eps}_+ \bar{Q}_- \highlight{-} \bar{\eps}_- \bar{Q}_+
}
...
\begin{definition}
	A \bam{superfield} is a function on superspace $\mbb{R}^{2|4}$.
\end{definition}
\begin{lemma}
...
\begin{definition}
A \bam{chiral superfield} $\Phi$ is  a superfield which obeys 
\eq{
\bar{D}_\pm \Phi = 0 
}
\end{definition}
...
\eq{
Q_ \pm &= \ev{\pd{\theta^\pm}}{x,\theta} + \ev{i\bar{\theta}^\pm \pd{x^\pm}}{\theta,\bar{\theta}} \\
&= \ev{\pd{\theta^\pm}}{\theta,\bar{\theta}} + \underbrace{\ev{\pd[y^\pm]{\theta^\pm}}{x,\theta}}_{=-i\bar{\theta}^\pm}\ev{\pd{y^\pm}}{\theta,\bar{\theta}} + i\bar{\theta}^\pm \ev{\pd{y^\pm}}{\theta,\bar{\theta}} \\
&= \ev{\pd{\theta^\pm}}{y,\theta}
}
and similarly 
\eq{
\bar{Q}_\pm = - \ev{\pd{\bar{\theta}^\pm}}{y,\theta} - \ev{2i \theta^\pm \pd{y^\pm}}{\theta,\bar{\theta}}
}
Using this, one find the component transformations 
\eq{
\delta \phi &= \eps_+ \psi_- - \eps_- \psi_+ \\
\delta \psi_\pm &= \eps_\pm F \pm 2i\bar{\eps}_\mp \del_\pm \phi \\
...
Let 
\eq{
K(\mc{F}_i, \Phi^a, \bar{\Phi}^a) \quad \text{(K\"ahler potential)}
}
be any $\mbb{R}$ smooth  function of $\mbb{R}$ superfield
\eq{
\mc{F}_i(x^\pm, \theta^\pm \bar{\theta}^\pm)
}
...
\eq{
\int_{\mbb{R}^{2|4}} K(\mc{F}_i, \Phi^a, \bar{\Phi}^a) \, d^2x \, d^2 \theta \, d^2\bar{\theta}
}
is known as a \bam{D-term}.
\end{definition}
\begin{prop}
...
\begin{definition}
Suppose $W(\Phi^a)$ (the \bam{superpotential}) is a holomorphic function of $\Phi^a$. An action of the form 
\eq{
	\int_{\mbb{R}^{2|2}} W(\Phi^a) \, d^2y \, d^2\theta
}
	is known as a \bam{F-term}.
\end{definition}
...
Let's consider now the simplest case of a single chiral superfield $\Phi$ and its conjugate $\bar{\Phi}$. \begin{definition}
	The \bam{Wess-Zumino} model in $1+1$-dimensions is the theory with $K(\Phi,\bar{\Phi}) = \bar{\Phi} \Phi$
and $W$ general, i.e.  
\eq{
...
\eq{
W^{(2)}(\Phi) = F W^\prime(\phi) - \psi_+ \psi_- W^{\prime \prime} (\phi)
}
...
&= \underbrace{\phi(x^\pm) -i \theta^+ \bar{\theta^+} \del_+\phi(x^\pm) - i \theta^- \bar{\theta}^- \del_- \phi(x^\pm) - \theta^+ \bar{\theta}^+ \theta^- \bar{\theta}^- \del_+ \del_- \phi(x^\pm)}_{(1)}\\
&\phantom{=} + \theta^+ \psi_+  (x^\pm) - i \theta^+ \theta_- \bar{\theta}^- \del_- \psi_+ (x^\pm) + \theta^- \psi_-(x^\pm) - i \theta^- \theta^+ \bar{\theta}^+ \del_+ \psi_- (x^\pm) + \theta^+ \theta^- F(x^\pm)
}
as a function on non-chiral superspace. 
Similarly, 
\eq{
\bar{\Phi}(x^\pm,\theta^\pm,\bar{\theta}^\pm) &= \bar{\phi}(x^\pm) + i \theta^+ \bar{\theta}^+ \del_+ \bar{\phi} + i \theta^- \bar{\theta}^- \del_- \bar{\phi} - \theta^+ \bar{\theta}^+ \theta^- \bar{\theta}^- \del_+ \del_- \bar{\phi} \\
&\phantom{=} - \bar{\theta}^+ \bar{\psi}_+(x^\pm) - i \bar{\theta}^+ \theta^- \bar{\theta}^- \del_- \bar{\psi}_+ - \bar{\theta}^- \bar{\psi}_- - i \bar{\theta}^- \theta^+ \bar{\theta}^+ \del_+ \bar{\psi}_- + \bar{\theta}^- \bar{\theta}^+ \bar{F}
}
we need to extract the $\theta^2 \bar{\theta}^2$ terms from $\bar{\Phi} \Phi$. We have 
\eq{
\bar{\Phi} \Phi|_{\theta^4} = - \bar{\phi} \del_+ \del_- \phi + \del_+ \bar{\phi} \del_- \phi + \del_- \bar{\phi} \del_+ \phi - \del_+ \del_- \bar{\phi} \phi + i \bar{\psi}_+ \del_- \psi_+ - i \del_- \bar{\psi}_+  \psi_+ + i \bar{\psi}_- \del_+ \psi_- - i \del_+ \bar{\psi}_- \psi_- + |F|^2
}
...
&= \int_{\mbb{R}^2} \psquare{\del^\mu \bar{\phi} \del_\mu \phi +i \bar{\psi}_+ \del_- \psi_+ + i\bar{\psi}_- \del_+ \psi_- + |F+\bar{W}^\prime|^2 - \abs{W^\prime}^2  - \psi_+ \psi_- W^{\prime\prime}(\phi) - \bar{\psi}_- \bar{\psi}_+ \bar{W}^{\prime\prime} ( \bar{\phi}) } \, d^2 x 
}
...
\eq{
G_\pm^0 &= 2 \del_\pm \bar{\phi} \psi_\pm \pm i \bar{\psi}_\mp F \\
G_\pm^1  &= \mp 2 \del_\pm \bar{\phi} \psi_\pm + i \bar{\psi}_\mp F
}
...
\begin{prop}
The WZ model has $U(1)_A$ \bam{axial R-symmetry} acting as 
\eq{
\Phi(x^\pm,\theta^\pm,\bar{\theta}^\pm) \to \bar{\Phi}(x^\pm, e^{\mp i \alpha} \theta^\pm, e^{\pm i \alpha} \bar{\theta}^\pm)
}
\end{prop}
\begin{proof}
...
\begin{prop}
Consider monomial $W(\Phi) = c\Phi^k$. The WZ model has  $U(1)_V$ \bam{vector symmetry} acting as 
\eq{
\Phi(x^\pm,\theta^\pm,\bar{\theta}^\pm) \to e^{iq\beta}\Phi(x^\pm, e^{-i\beta} \theta^\pm , e^{i\beta} \bar{\theta}^\pm)
}
...
\eq{
V(\phi^,\bar{\phi}^a) = \sum_a \abs{\del_a W}^2 
}
...
\eq{
\forall a, \, \del_a W\pround{\phi_0^a} = 0
}
In a quantum theory these values $\phi_0^a$ are the possible VEVs of the scalars. 
\begin{definition}
	The set $\mc{M}$ of all solutions to the vacuum conditions is called the \bam{vacuum moduli space}. If $\mc{M}$ is not discrete we say the potential has \bam{flat directions}.
\end{definition}

\begin{example}
	Consider one field with $W(\phi)= \frac{1}{2}m\phi^2 + \frac{1}{3}\lambda \phi^3$. Then 
	\eq{
\mc{M} = \pbrace{\phi=0, \, \phi = -\frac{m}{\lambda}}	
}
\end{example}
\begin{example}
	Consider 3 scalar fields with $W(\phi) = \phi^1 \phi^2 \phi^3$. Then 
	\eq{
\mc{M} = \pbrace{\phi^2 = 0 = \phi^3} \cup 	\pbrace{\phi^1 = 0 = \phi^3} \cup \pbrace{\phi^2 = 0 = \phi^1}
}
\end{example}
...
With $n$ fields the vacuum moduli space is the affine variety 
	\eq{
\mc{M} = \faktor{\mbb{C}[\phi^1, \dots, \phi^n]}{(\del_a W)}	
}
\end{prop}
...
\eq{
K_M + K_\Lambda = \frac{1}{\eps} \pround{M \bar{M} + \Lambda \bar{\Lambda}}
}
...
\eq{
W_{eff}(\Phi,M,\Lambda) = M\Phi^2 f\pround{\frac{\Phi\Lambda}{M}}
}
...
\begin{definition}
	An \bam{almost complex structure} $J$ is a map 
	\eq{
...
\begin{definition}
Given an almost complex structure $J$, define the \bam{holomorphic and antiholomorphic tangent bundles} on $M$ by 
	\eq{
		T^{(1,0)}_p M &= \set{X \in T_pM \otimes \mbb{C} : \frac{1}{2}(1-iJ)X = X} \\
		T^{(0,1)}_pM &= \set{X \in T_pM \otimes \mbb{C} : \frac{1}{2}(1+iJ)X = X}
	}
...
\begin{definition}
	Given an almost complex structure, the \bam{Hodge decomposition} is the decomposition 
	\eq{
		\Omega^k(M,\mbb{C}) = \bigoplus_{k = p+q} \Omega^{(p,q)}(M)
	}
where 
\eq{
\Omega^{(p,q)}(M) = \bigwedge^p T^{\ast (1,0)}M \wedge \bigwedge^q T^{\ast (0,1)}M 
}
i.e. 
\eq{
...
\begin{definition}
	...
	\eq{
		\forall X,Y \in TM \otimes \mbb{C}, \; (1+iJ) \comm[(1-iJ)X]{(1-iJ)Y} = 0
	}
	i.e. the Lie bracket of two holomorphic vector fields is holomorphic.
\end{definition}

\begin{definition}
	Given an almost complex structure, the \bam{Nijenhuis tensor} is defined by 
	\eq{
	 N(X,Y) = -J^2(\comm[X]{Y}) + J\pround{\comm[JX]{Y} + \comm[X]{JY}} - \comm[JX]{JY}
}
\end{definition}
...
\begin{definition}
	$J$ is a \bam{complex structure} iff it is an integrable almost complex structure. 
\end{definition}
...
\begin{definition}
A \bam{K\"ahler manifold} is a manifold $M$ with three compatible structures: a Riemannian metric $g$, a positive symplectic form $\omega$, and a complex structure $J$.
\end{definition}
...
\begin{prop}
	...
	\eq{
\forall X,Y \in TM \otimes \mbb{C}, \, g(X,Y) = \omega(X,JY)	
}
\end{prop}
\begin{proof}
	$\omega$ is non-degenerate, and $J$ has non-zero eigenvalues, so $g$ is too. Then 
	\eq{
g(Y,X) = \omega(Y,JX) = -\omega(JX,Y) = \omega(JX,J^2 Y)	 = \omega(X,JY)= g(X,Y)
}
...
\end{proof}
\begin{corollary}
	Locally we may find a \bam{K\"ahler potential} $K(z,\bar{z})$ s.t. 
	\eq{
\omega = i \del \bar{\del} K	
}
defined up to transformations 
\eq{
K(z,\bar{z}) = K(z,\bar{z}) + f(z) + \bar{f}(\bar{z})
}
\end{corollary}
\begin{proof}
...
\begin{prop}
	...
	\eq{
g_{a\bar{b}} = \del_a \del_{\bar{b}} K 	
}
and the non-vanishing connection components are 
\eq{
\Gamma^a_{bc} &= g^{a\bar{d}}\del_b g_{c\bar{d}} \\
\Gamma^{\bar{a}}_{\bar{b}\bar{c}} &= g^{\bar{a}d}\del_{\bar{b}} g_{d\bar{c}} 
}
\end{prop}
...
\begin{definition}
	The metric on $\mbb{CP}^n$ coming from the K\"ahler potential $K = \log \pround{1+\sum_{a=1}^n \abs{z^a}^2}$ is called the \bam{Fubini-Study} metric.
\end{definition}
...
\eq{
\int_{\mbb{R}^{2|4}} K(\Phi,\bar{\Phi}) d^4 \theta \, d^2 x
}
which depends on $K$ up to changes 
\eq{
K(\Phi,\bar{\Phi}) \to K(\Phi,\bar{\Phi}) + f(\Phi) + \bar{f}(\bar{\Phi})
}
Doing the fermionic integral we find the kinetic part of the action is 
\eq{
S_{kin}[\Phi,\bar{\Phi}] &= \int_{\mbb{R}^2}\left[ - g_{a\bar{b}}\del^\mu \phi^a \del_\mu \bar{\phi}^{\bar{b}} + ig_{a\bar{b}}\bar{\psi}_+^{\bar{b}} \nabla_- \psi_+^a + ig_{a\bar{b}} \bar{\psi}^{\bar{b}}_- \nabla_+ \psi_-^a \right. \\
&\phantom{=} \quad \left. + R_{a\bar{b} c \bar{d}} \psi^a_+ \psi^c_- \bar{\psi}^{\bar{b}}_- \bar{\psi}^{\bar{d}}_+ + g_{a\bar{b}} \pround{F^a - \Gamma^a_{cd} \psi^c_+ \psi^d_-} \pround{ \bar{F}^{\bar{b}} - \Gamma^{\bar{b}}_{\bar{c}\bar{d}}\bar{\psi}^{\bar{c}}_- \bar{\psi}^{\bar{d}}_+ } \right] d^2 x 
}
where $\nabla_\mu \psi_\pm^a = \del_\mu \psi^a_\pm + \Gamma^a_{bc} \del_\mu \phi^b \psi^c_\pm$. Again the potential term is 
\eq{
\frac{1}{2} \int_{\mbb{R}^{2|2}} \psquare{F^a \pd[W]{\phi^a}- \psi_+^a \psi_-^b \frac{\del^2 W}{\del \phi^a \del \phi^b}} \, d^2x + \text{complex conjugate }
}
leaving $F$ to be auxiliary with 
\eq{
F^a = \Gamma^a_{bc} \psi^c_+ \psi^d_- - \frac{1}{2} g^{a\bar{b}}\del_{\bar{b}} \bar{W}
}
The full action is then 
\eq{
S[\phi,\psi] &= \int_{\mbb{R}^2}\left[  - g_{a\bar{b}}\del^\mu \phi^a \del_\mu \bar{\phi}^{\bar{b}} + ig_{a\bar{b}}\bar{\psi}_+^{\bar{b}} \nabla_- \psi_+^a + ig_{a\bar{b}} \bar{\psi}^{\bar{b}}_- \nabla_+ \psi_-^a  + R_{a\bar{b} c \bar{d}} \psi^a_+ \psi^c_- \bar{\psi}^{\bar{b}}_- \bar{\psi}^{\bar{d}}_+ \right. \\
&\phantom{=} \quad \left. -\frac{1}{4} g^{a\bar{b}} \del_a W \del_{\bar{b}} \bar{W} - \frac{1}{2} \nabla_a \del_b W \psi^a_+ \psi^b_- - \frac{1}{2} \nabla_{\bar{a}} \del_{\bar{b}} \bar{W} \bar{\psi}^{\bar{a}}_- \bar{\psi}^{\bar{b}}_+ \right] \, d^2 x 
}

\begin{remark}
We may consider general Riemann surface $\Sigma$ rather than just $\mbb{R}^2$ by making the replacement 
\eq{
d^2 x &\to \sqrt{h} d^2 x \\
g_{a\bar{b}} \del^\mu \phi^a \del_\mu \bar{\phi}^{\bar{b}} &\to g_{a\bar{b}} h^{\mu\nu} \del_\mu \phi^a \del_\mu \bar{\phi}^{\bar{b}}
}
...
\eq{
\psi_\pm \in \Gamma \pround{\Sigma, \phi^\ast T^{(1,0)}N \otimes S_\pm}
}
...
\begin{definition}
	A classical symmetry is \bam{anomalous} if it is not preserved in the quantum theory.  
\end{definition}
...
\begin{example}
	...
	\eq{
S = i \int_{T^2} \psquare{\bar{\psi}_+ D_z \psi_++ \bar{\psi}_- \bar{D}_{\bar{z}} \psi_- } dz
}
where $z=x+iy$ is a coordinate on the torus and $D_z=\del_z + A_z$, $A$ the gauge field. This has the global symmetry
\eq{
\psi_\pm &\mapsto e^{-i(\alpha\pm\beta)}\psi_\pm \\
\bar{\psi}_\pm &\mapsto e^{+i(\alpha\pm\beta)}\bar{\psi}_\pm \\
A&\mapsto A 
}
Suppose that the gauge field has non-zero instanton number, i.e. 
\eq{
k = \int_{T^2} c_1(E) = \frac{i}{2\pi} \int_{T^2} F > 0
}
then by the index theorem 
\eq{
\dim\pround{\ker \bar{D}_{\bar{z}}} - \dim\pround{\ker D_z} = k
}
...
\eq{
k = \int_{T^2} c_1 \pround{\phi^\ast T^{(1,0)}M} = \frac{i}{2\pi} \int_{T^2} \tr(R)
}
where $R$ is the curvature 2-form  of the connection in $M$ pulled back to $T^2$. 
\begin{definition}
	A \bam{Calabi-Yau} manifold is a K\"ahler manifold where $\tr(R)=0$. 
\end{definition}
...
\eq{
S[\phi,\psi] = \int_{\Sigma } \left[  - g_{a\bar{b}} h^{\mu\nu} \del_\mu \phi^a \del_\mu \bar{\phi}^{\bar{b}} + ig_{a\bar{b}}\bar{\psi}_+^{\bar{b}} \gamma^\mu \nabla_\mu \psi_+^a +  + R_{a\bar{b} c \bar{d}} \psi^a_+ \psi^c_- \bar{\psi}^{\bar{b}}_- \bar{\psi}^{\bar{d}}_+ \right] \sqrt{h} \, d^2 x
}
is invariant under the scaling 
\eq{
h_{\mu\nu} &\mapsto \lambda^2 h_{\mu\nu}, & \gamma^\mu &\mapsto \lambda^{-1} \gamma^\mu, & \phi^a &\mapsto \phi^a, & \psi^a_\pm 7\mapsto \lambda^{-\frac{1}{2}} \psi^a_\pm 
}
...
\eq{
f(g,h) = \pangle{(\psi_-)^k (\bar{\psi}_+)^k}_h
}
...
\eq{
A(g) &= i \int_{T^2} g_{a \bar{b}}\pround{\del_z \bar{\phi}^{\bar{b}} \del_{\bar{z}} \phi^a + \del_{\bar{z}} \bar{\phi}^{\bar{b}} \del_z \phi^a} \, dz \wedge d\bar{z} \\
&= 2i \int_{T^2} g_{a \bar{b}}\pround{\del_z \bar{\phi}^{\bar{b}} \del_{\bar{z}} \phi^a} dx \wedge d\bar{z} + \int_{T^2} \phi^\ast \omega 
}
and so for holomorphic $\phi$ we get $A(g) = \int_{T^2} \phi^\ast \omega$. Hence under scaling $h \mapsto \lambda^2 h$ we get 
\eq{
[\omega] \mapsto [\omega^\prime] =[\omega] - \frac{i}{2\pi} \tr(R) \log\lambda
}
\end{example}
\begin{remark}
...
\eq{
g_{ij}(\phi_0 + \xi) = \delta_{ij} - \frac{1}{2} R_{ijkl}(\phi_0) \xi^k \xi^l + \mc{O}(\xi^3)
}
so to lowest order the action about this point is 
\eq{
S[\xi] = \frac{1}{2} \int_{\Sigma }\psquare{\del^\mu \xi^i \del_\mu \xi_i - \frac{1}{3} R_{ijkl}(\phi_0) \xi^k \xi^l \del^\mu \xi^i \del_\mu \xi^j + \mc{O}(\xi^5)} \, d^2 x 
}
...
\eq{
\pangle{\xi^i(x)\xi^j(y)}^{(1)} = \int \frac{d^2k}{(2\pi)^2} \frac{e^ik\cdot(x-y)}{k^2} \psquare{\delta^{ij} + \frac{1}{3} \int \frac{d^2 p}{(2\pi)^2} \frac{1}{p^2}R^{ij}(\phi_0)}
}
The loop integral is divergent, and we can regularise by introducing UV and IR cutoffs to get  
\eq{
\int_{\mu \leq \abs{p} \leq \Lambda} \frac{d^2p}{(2\pi)^2} \frac{1}{p^2} = \frac{1}{2\pi}\int_\mu^\Lambda \frac{dp}{p} = \frac{1}{\pi} \log \frac{\Lambda}{\mu}
}
Comparing this to the classical propagator we can absorb the UV divergence if we include counter terms s.t. the metric is 
\eq{
g_{ij} + \delta g_{ij}(\Lambda) = \delta_{ij} - \frac{1}{6\pi} R_{ij}(\phi_0) \log \frac{\Lambda}{\lambda}
}
which leaves
\eq{
g(\lambda,\mu)\delta_{ij} + \frac{1}{6\pi} R_{ij}(\phi_0) \log \frac{\lambda}{\mu}
}
This runs with 
\eq{
\beta_{ij} = \lambda \frac{dg_{ij}}{d\lambda}= \frac{1}{6\pi} R_{ij}
}
