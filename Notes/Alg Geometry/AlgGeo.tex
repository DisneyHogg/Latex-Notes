...
\begin{definition}
	We define $\text{Spin}_n$ be the universal cover of $SO(n)$ for $n>2$. This is a double-cover and has the SES 
	\eq{
0 \to \mbb{Z}_2 \to \text{Spin}_n \overset{\pi}{\to} SO(n) \to 0	
}
\end{definition}
...
We will now want to understand two papers, by Atiyah (\hl{ref}) and Kallel, Sjerve (\hl{ref}), on spin structure on Riemann surfaces. 
\subsection{Further results on square roots}
Recall in subsection \ref{subsec:Picard Group} we looked at solutions in the divisor class group to the equation $2D=K$. We can say some more:
...
\begin{theorem}
	The function $\phi:S(X) \to F_2, \, \phi(L) = \dim \Gamma(L)$ is a quadratic function whose associated bilinear form is the cup product on $H^1(X,F_2)$. 
\end{theorem}
