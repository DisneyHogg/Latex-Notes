...
abusing notation slightly. In this form the Lax pair becomes a statement about commuting operators 
	\eq{
	\comm[(-\phi+iD_1) +\zeta(D_3-iD_2)]{(D_3+iD_2) + 2i\zeta D_1 + \zeta^2(D_3-iD_2)}=0	
}
The kernel of $(-\phi+iD_i) +\zeta(D_3-iD_2)$ for each $\zeta$ combines to give a (rank 2) vector bundle $E \to \mbb{P}^1$, and the commutation with the complex structure $I_\zeta$ means that it is holomorphic.  
\end{remark}
...
\begin{example}[Prasad-Sommerfield solution \cite{Prasad1975}]
	...
	\eq{
	\phi &= ih(r) \frac{x^i \sigma_i}{r} = \frac{-2h(r)x^iT_i}{r} = \frac{H(r)x^i T_i}{r^2} & A_i &= -\frac{i}{2}\psquare{1-k(r)} \frac{\eps\indices{_j_j^k} x^j \sigma_k}{r^2} =[1-k(r)]\frac{\eps_{ijk}x^j T^k}{r^2}
}
...
\eq{
F &= \frac{1}{2} F_{jk} \star(dx^j \wedge dx^k) \\
...
\hl{Be careful of scaling and conventions here, and maybe choose a better name for the function other than $k$.}
\end{example}
...
Nahm established an equivalence between non-singular charge-$n$ $SU(2)$-monopoles and data $\pbrace{T_i(s) \, | \, T_i \in M_n(\mbb{C}), \, s \in [0,2]}$ subject to 
\begin{enumerate}
