...
then the additional conditions in the above theorem may be restated as conditions on the transition function $F$ that $\det F =1$ and $F^\dagger=F$ (up to a similarity condition, that is $F = \underline{K} \tilde{F} K^{-1}$ for some unit-determinant holomorphic matrices, and $\tilde{F}$ satisfies these conditions).  
\end{remark}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{\secmath{SU(2)} ansatze}

\begin{definition}
	A rank-2 vector bundle $E \to M$ is an \bam{extension} of a line bundle $L_1$ if $\exists L_2$ a line bundle with SES
	\eq{
...
Now line bundles are classified by their Chern class, and if we let $k = -c_1(L_1)$ and \hl{recall that the line bundle on the sphere with transition function $z \mapsto z^n$ has Chern class $-n$} we know that 
\eq{
...
\end{remark}
\begin{lemma}
	There is no solution when $k<0$. 
\end{lemma}
When $k>0$ we have a chance to construct a solution from a splitting solution $g=\underline{h}-h$ as found in the abelian case and then some additional information. This is made explicit in the following result:
\begin{theorem}
...
Suppose that $B_a, \Delta_r$ are holomorphic on the region $U \subset M_\mbb{C}$. Define the matrix $k\times k$ matrix $M$ by $M_{rs} = \Delta_{r+s-k-1}$ (assumed invertible) and let 
\eq{
E = (M^{-1})_{11}, \; F=(M^{-1})_{1k} = (M^{-1})_{k1}, \; G=(M^{-1})_{kk} 
}
and define a gauge potential $\Phi$ by 
\eq{
\Phi_{A 0^\prime} &= \frac{1}{2F} \begin{pmatrix} \hat{\del}_{A0^\prime} F & 0 \\ -2\hat{\del}_{A1^\prime} G & -\hat{\del}_{A0^\prime} F \end{pmatrix} \\
\Phi_{A 1^\prime} &= \frac{1}{2F} \begin{pmatrix} -\hat{\del}_{A1^\prime} F & -2\hat{\del}_{A0^\prime} E \\ 0 & \hat{\del}_{A1^\prime} F \end{pmatrix}
}
