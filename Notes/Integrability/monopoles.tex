...
\eq{
\Omega = \begin{pmatrix} 1 & \zeta^{-15} & -\zeta^{-5} & -\zeta^{-5-15} \\ 1 & \zeta^{-12} & -\zeta^{-4} & -\zeta^{-4-12}\end{pmatrix} = \begin{pmatrix} 1 & -1 & -\zeta & \zeta \\ 1 & 1 & -\zeta^2 & -\zeta^2 \end{pmatrix}
}
and so 
\eq{
...
\hl{These numerical results seem to suggest the analytic answer is wrong, and that I should really be finding}
	\eq{
		\Omega \propto \begin{pmatrix} 1 & -1 &  \zeta^2  & -\zeta^2 \\ 1 & 1 & \zeta & \zeta \end{pmatrix}
	}	
giving 
\eq{
...
\begin{remark}
	Because of the action of the modular group on $\Omega$, it is difficult to inspect the computed answer to see if it is correct. This is not true of $\tau$, which i
\end{remark}
...
with $n\times k$ quaternionic matrix $\Lambda$ and $k\times k$ constant quaternioninc matrix $B$. We have to solve the non-linear condition 
\eq{
