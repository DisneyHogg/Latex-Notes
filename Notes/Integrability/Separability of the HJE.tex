...
\eq{
4R_{ij} &= \sum_{k \neq i,j} \psquare{(\del_i \log g_{jj} - \del_i)\del_j \log g_{kk} + (j \leftrightarrow i) - (\del_i \log g_{kk})(\del_j \log g_{kk})} \quad (i \neq j) \\
4 R_{ii} &= \left(\partial_{i} \log g_{ii}-2 \partial_{i}\right) \partial_{i} \log \frac{g}{g_{ii}}-\sum_{k \neq i}\left[\left(\partial_{i} \log g_{kk}\right)^{2}+\left(\partial_{k} \log \frac{g}{g_{ii}^{2}}+2 \partial_{k}\right) g^{kk} \partial_{k} g_{ii}\right] \quad (\text{n.s. over }i)
}
...
\eq{
4R_{ij} = \sum_{k \neq i,j} \psquare{(\del_i \log g^{jj} + \del_i)\del_j \log g^{kk} + (j \leftrightarrow i) - (\del_i \log g^{kk})(\del_j \log g^{kk})}
}
To do this we have used 
\eq{
\del_i \log g_{jj} +\del_i \log g^{jj} &= \del_i \log g_{jj}g^{jj} \quad (\text{n.s. over }j)\\
&= 0 \\
\Rightarrow \del_i \log g_{jj} &= - \del_i \log g^{jj}
}
...
\eq{
(\del_i \log g^{jj})(\del_j \log g^{kk}) + (\del_j \log g^{ii})(\del_i \log g^{kk}) - (\del_i \log g^{kk})(\del_j \log g^{kk}) &= \frac{\del_i \del_j g^{kk} - S_{ij}(g^{kk})}{g^{kk}} - \frac{ (\del_i g^{kk})(\del_j g^{kk})}{(g^{kk})^2} \\
&= \del_i \del_j \log g^{kk}
}
imposing the LCSCs  $S_{ij}(g^{kk})=0$. Hence 
\eq{
R_{ij} = \frac{3}{4} \sum_{k \neq i,j} \del_i \del_j \log g^{kk}
}
\end{proof}
...
\eq{
g^{aa} &= \frac{\psi_a }{\phi_1 + \phi_2} \quad (a=1,2)\\
g^{33} &= \frac{\Psi_1 + \Psi_2}{\phi_1 + \phi_2}
}
where $\psi_a, \Psi_a, \phi_a$ are functions of $q^a$ only. Here we can calculate
\eq{
R_{12} &= \frac{3}{4} \del_1 \del_2 \log \pround{\frac{\Psi_1 + \Psi_2}{\phi_1 + \phi_2}} \\
&= \frac{3}{4} \del_1 \del_2 \psquare{\log(\Psi_1 + \Psi_2) - \log(\phi_1 + \phi_2)} \\
&= \frac{3}{4}\psquare{\frac{\phi_1^\prime \phi_2^\prime}{(\phi_1 + \phi_2)^2} - \frac{\Psi_1^\prime \Psi_2^\prime}{(\Psi_1 + \Psi_2)^2}} \\
R_{33} &= -\frac{1}{4}\sum_a \pround{\del_a \log \frac{g}{g_{aa}^2} + 2\del_a}\pround{g^{aa} \del_a g_{33}} \\
&= -\frac{1}{4} \left\lbrace \pround{\del_1 \log \frac{\psi_1(\phi_1 + \phi_2)}{\psi_2(\Psi_1 + \Psi_2)} + 2\del_1}\pround{\frac{\psi_1}{\phi_1 + \phi_2} \del_1 \frac{\phi_1 + \phi_2}{\Psi_1 + \Psi_2}}\right.
}
...
\begin{lemma}
	...
	\eq{
\bar{R}_{ab} &= R_{ab} - \frac{3(n-2)}{4}f_{ab} \quad (a \neq b)
}
\end{lemma}
\begin{proof}
	Using 
	\eq{
	R_{ab} = \frac{3}{4}\psquare{\pround{\sum_{c \neq a,b} \del_a \del_b \log g^{cc}} + \del_a \del_b \log \det \abs{g^{\alpha\beta}}}
}
we get 
\eq{
\bar{R}_{ab} &=  \frac{3}{4}\psquare{\pround{\sum_{c \neq a,b} \del_a \del_b \log e^{-f} g^{cc}} + \del_a \del_b \log \det \abs{e^{-f}g^{\alpha\beta}}} \\
&=R_{ab} + \frac{3}{4}\psquare{\pround{\sum_{c \neq a,b} \del_a \del_b \log e^{-f}} + \del_a \del_b \log  e^{-rf}} \\
&= R_{ab} + \frac{3}{4}(n-r-2 + r)f_{ab}
}
\end{proof}
...
\begin{definition}
	In dimensions $n \geq 3$ the \bam{conformal Laplacian} is the operator $L$ defined by 
	\eq{
...
\begin{definition}
A \bam{separable Killing algebra} or \bam{r-separability structure} is a pair $(D,\mc{K})$ where 
\begin{itemize}
