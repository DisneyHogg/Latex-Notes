...
\begin{prop}
	...
	\eq{
\comm[\frac{d}{ds} + M]{A} = 0	
}
where 
\eq{
A(\zeta) &= (T_1 + iT_2) - 2i T_3 \zeta + (T_1 -iT_2) \zeta^2 \\
...
this Lax pair can be rewritten as 
\eq{
A(\zeta) &= \beta -(\alpha + \alpha^\dagger)\zeta -\beta^\dagger \zeta^2 \\
...
\begin{prop}
	...
	These are the affine Toda field equations if we make the normalisation $\kappa(E_\alpha,E_{-\alpha}) = \frac{m^2}{\lambda^2}$ . 
\end{prop}
\begin{prop}
...
\begin{equation*}
\alpha=T_4+ i T_3,\quad \beta = T_1+iT_2,
\quad
