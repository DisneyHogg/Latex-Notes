...
\end{itemize}
We call $\Pi$ the \bam{simple roots}, and $\Pi^\vee$ the \bam{simple coroots}
\end{definition}
...
\begin{idea}
	...
	\eq{
		A \to (\mf{h},\Pi^\vee,\Pi) \to \tilde{\mf{g}}(A) \to \mf{g}(A)=\faktor{\tilde{\mf{g}}(A)}{r_{max}}
	}
\end{idea}
...
\eq{
\tilde{\mf{g}}_\alpha \equiv \pbrace{x \in \tilde{\mf{g}}(\Sigma) \, | \, \forall h \in \mf{h}, \, \comm[h]{x} = \alpha(h)x}
}
\end{definition}
...
\begin{definition}
The \bam{Kac-Moody} Lie algebra associated to $\Sigma$ is $\mf{g}(\Sigma)\equiv \faktor{\tilde{\mf{g}}(\Sigma)}{r}$
\end{definition}
...
This descends to a map $w:\mf{g}(A) \to \mf{g}(A)$ called the \bam{Chevalley involution}.
\end{definition}
...
\begin{proof}
	...
	\eq{
		J_x \equiv \sum_{k \geq 0} \ad(\mf{g}_1)^k(x) \ni x
	}
	Then we can note 
	\begin{itemize}
...
\begin{lemma}
	...
	\eq{
h \cdot v_m &= (\lambda - 2m)v_m \\
e \cdot v_m &= (\lambda -m+1)v_{m-1}
}
\end{lemma}
\begin{proof}
	Note 
	\eq{
hf^m &= (\comm[h]{f}+fh)f^{m-1} \\
&= -2f^m + fhf^{m-1}.	
}
and so so repeating this procedure we get 
\eq{
hf^m = f^m(h-2m) \Rightarrow hf^m \cdot v= (\lambda-2m) \cdot v
}
We do the same with $e$ seeing 
\eq{
ef^m = (h+fe)f^{m-1} &= hf^{m-1}+fhf^{m-2} + \dots + f^{m-1}h+f^me \\
&= f^{m-1}\psquare{(h-2m+2) + (h-2m+4)+ \dots + (h)} + f^m e \\
&= f^{m-1}\psquare{m(h-m+1)}+f^me
}
yielding the result
\end{proof}
\begin{comment}
...
\begin{definition}
	Given $V \in \Rep(\mf{g}), \, x \in \mf{g}$ we say $x$ acts \bam{locally nilpotently} if $\forall v \in V, \exists N$ s.t. $x^N \cdot v =0$.
\end{definition}
...
The \bam{fundamental chamber} is $\mc{C} \equiv \pbrace{h \in \mf{h}_\mbb{R} \, | \, \alpha_i(h) \geq 0}$. Correspondingly the \bam{Tits cone} is $X = \bigcup_{w \in W} w(\mc{C})$
\end{definition}
...
\eq{
(\delta,\alpha_0) &= 0 = (\delta,\alpha_1) \\ \Rightarrow (\delta,\delta) &= 0  \quad \text{but} \quad (\delta,\Lambda) = 1\\ 
(c,\alpha_0^\vee) &= 0 = (c,\alpha_1^\vee) \\
\Rightarrow (c,c) &= 0 \quad \text{but} \quad (c,d) = 1
}
Now we have orthogonal decompositions 
\eq{
\hat{\mf{h}} &= \mbb{C} \alpha_1^\vee \oplus \mbb{C}c \oplus \mbb{C}d \\
...
\eq{
\hat{g} = \mc{L} \mf{g} \oplus \mbb{C}c \oplus \mbb{C}d
}
s.t. 
\begin{itemize}
...
\begin{definition}
Let $\mf{a}$ be a Lie algebra. A \bam{central extension} if $\mf{a}$ is a pair $(\tilde{\mf{a}},\pi)$ s.t. 
\begin{itemize}
